...
namespace SJSApp
{
    class SJSLoginManager
    {
...
#else
            return (_iosUser == null ? null : new string[] { _iosUser, _iosPass });
#endif
        }
        ...
        private void GenerateNewToken(Action<bool> callback)
        {
            GetAntiAjaxToken(async (string resp) =>
            {
                ...
                using (WebResponse response = await request.GetResponseAsync())
                {
                    ...
                    Newtonsoft.Json.Linq.JObject o = JsonConvert.DeserializeObject<dynamic>(responseContent);
                    // dont ask me why but u need this for it not to crash
                    bool pls = (bool)o["LoginSuccessful"];
        ...
        public async void MakeAPICall(string call, Action<Newtonsoft.Json.Linq.JArray> callback)
        {
            ...
            {*/
            request = WebRequest.Create(BASE_URL + "api/" + call);
            request.Method = "GET";
